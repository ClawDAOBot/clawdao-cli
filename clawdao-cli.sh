...
cmd_create() {
    ...
    echo "✅ Task created in project $project! Metadata: $IPFS_GATEWAY/$cid"
}
...
cmd_request_tokens() {
    ...
    echo "✅ Token request created for $amount PT!"
}
...
cmd_approve_request() {
    ...
    echo "✅ Request #$id approved - PT minted to requester!"
}
...
QUICK ALIASES
  s     → status          t     → tasks           p     → proposals
  m     → members         b     → balance         c     → claim
  v     → vote            mt    → my-tasks        sub   → submit
  done  → complete        new   → create          props → proposals
